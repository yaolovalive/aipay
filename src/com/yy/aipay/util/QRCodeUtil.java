...
public class QRCodeUtil {
	 ...
	 * @param filePath
	 *            文件保存路径
	 * @return 二维码文件名(带后缀,不带路径)
	 */
	public static String createQrcode(String text, String filePath) {
		try {
			int qrcodeWidth = 300;
			...
			File QrcodeFile = new File(filePath + File.separator + UUID.randomUUID() + "." + qrcodeFormat);
			ImageIO.write(image, qrcodeFormat, QrcodeFile);
			MatrixToImageWriter.writeToFile(bitMatrix, qrcodeFormat, QrcodeFile);
			return QrcodeFile.getName();
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
